...
for( i in 1:nrow(US)) {
  ...
  Total <- nrow(AF)
  cellarea <- (pi*20^2)/Total
  AF <- AF %>% filter(LandClass == 190) %>% summarize(Count=n(),
                                                      Area=Count*cellarea,
                                                      ScaleLength = Area^.5,
                                                      Total=Total) %>% mutate(ID=i) %>%mutate(BEid=US$Bid[i])
...
DF$PopCount[is.na(DF$PopCount)]<-0
ggplot(DF,aes(Area))+geom_histogram()
ggplot(DF,aes(x=Area,y=PopCount))+geom_point()+geom_smooth()
ggplot(DF,aes(x=Area,y=PopCount,fill=Country))+geom_point()+geom_smooth()+facet_grid(~Country)

ggplot(filter(DF,Country=="China"),aes(x=Area,y=PopCount))+geom_point()+geom_smooth()
ggplot(filter(DF,Country=="Canada"),aes(x=Area,y=PopCount))+geom_point()+geom_smooth()

ggplot(filter(DF,Country=="India"),aes(x=Area,y=PopCount))+geom_point()+geom_smooth()


ggplot(filter(DF,Country=="United States"),aes(x=Area,y=PopCount))+geom_point()+geom_smooth()

ggplot(filter(DF,Country=="Sweden"),aes(x=Area,y=PopCount))+geom_point()+geom_smooth()

ggplot(filter(DF,Country=="Russia"),aes(x=Area,y=PopCount))+geom_point()+geom_smooth()
ggplot(filter(DF,Country=="Korea, South"),aes(x=Area,y=PopCount))+geom_point()+geom_smooth()

ggplot(filter(DF,Country=="United Kingdom"),aes(x=Area,y=PopCount))+geom_point()+geom_smooth()

ggplot(filter(DF,Country=="Australia"),aes(x=Area,y=PopCount))+geom_point()+geom_smooth()

ggplot(filter(DF,Country=="Brazil"),aes(x=Area,y=PopCount))+geom_point()+geom_smooth()
