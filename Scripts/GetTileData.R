...
for( i in 1:nrow(CRN)) {
 ...
 S <- left_join(S,CRN, by =  "Id")
                                               
  
 write.csv(S, "CRN30mDistance.csv")
 
 S2 <- S  %>% group_by(Id) %>% summarize(Area=sum(Area),
                                         ScaleLength= Area^.5)
 
 S2 <- left_join(S2,CRN,by="Id")
 
 write.csv(S2, "CRN30mSummary.csv")
 
 DFin <- read.csv("CRNurban300.csv",stringsAsFactors = F)
 ...
 write.csv(DFin, "CRNPopAndArea.csv")
